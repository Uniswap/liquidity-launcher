...
contract DeployGovernedLBPStrategyFactoryScript is Script, Parameters {
    function run() public {
        DeployParameters memory params = getParameters(block.chainid);
        bytes32 initCodeHash = keccak256(
            abi.encodePacked(
                type(GovernedLBPStrategyFactory).creationCode, abi.encode(params.positionManager, params.poolManager)
            )
        );
