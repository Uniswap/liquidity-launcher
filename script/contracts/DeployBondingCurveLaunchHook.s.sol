...
contract DeployBondingCurveLaunchHookScript is Script, Parameters {
    ...
    function predict(address authorized, address tokenJar) public view returns (address hookAddress, bytes32 salt) {
        ...
        return HookMiner.find(
            ...
            abi.encode(params.poolManager, params.positionManager, authorized, tokenJar)
        );
    }
    ...
    /// @param authorized The strategy authorized to initialize curve pools.
    /// @param tokenJar The recipient of the curve's accrued fees swept at graduation.
    function run(address authorized, address tokenJar) public returns (address) {
        DeployParameters memory params = getParameters(block.chainid);
        (address hookAddress, bytes32 salt) = predict(authorized, tokenJar);
        ...
        BondingCurveLaunchHook hook =
            new BondingCurveLaunchHook{salt: salt}(params.poolManager, params.positionManager, authorized, tokenJar);
