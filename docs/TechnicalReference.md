...
#### FeeSplitter
The `FeeSplitter` is a singleton custodian of native-ETH v4 LP positions with immutable, deploy-time fee splits. `collectFees(uint256[] tokenIds)` permissionlessly collects each position's accrued fees and pushes independent basis-point splits of the native ETH side (`currency0`) and the token side (`currency1`) to fixed recipients. A fee-beneficiary sentinel recipient resolves per position: the beneficiary registered when the position was deposited (positions safe-transferred through the PositionManager may carry the abi-encoded beneficiary as transfer data) takes priority, falling back to the UERC20 `creator()` of the pool's token. If neither resolves or a native send to the beneficiary fails, that share is routed to an immutable per-side fallback so a collect can never be blocked. Launcher-created tokens report the launcher itself as `creator()`, so `DirectLaunchStrategy` requires the original creator in `configData` and proves the claim against the token's graffiti (the launcher's onchain commitment to its `createToken` caller) before registering them as beneficiary. Tokens created through an intermediary (e.g. a generic multicall aggregator) can only prove the intermediary — creators must call the launcher directly to receive creator fees. Positions held by the splitter are irrecoverable by design — there is no owner, operator, or exit path.
