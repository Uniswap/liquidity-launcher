...
2. **Tier 2 — `tryMigrate` retry**: retries the same migration function with the same `MigratorParameters`, except `positionDefinitions` is overwritten in memory with a single full-range LP definition. The configured `hook`, fee, tick spacing, LP allocation schedule, `supplyForLP`, and recipients are preserved. If the original failure was caused by the configured hook or pool initialization itself, the retry is expected to fail the same way and fall through to release.

3. **Tier 3 — `_release`**: the final safety net. Reached only if both migration attempts revert. Best-effort sweeps the initializer's currency, then releases whatever the strategy can move to `leftoverRecipient` and emits `FundsRecovered`. Funds are not permanently locked even if both prior tiers fail unexpectedly.

Outcomes are observable from three distinct events: `Migrated` (tier 1 success), `FallbackMigrated` (tier 2 success), or `FundsRecovered` (tier 3 release).
