...
- **FeeSplitter** — zero-admin custodian of v4 LP positions pairing its configured quote currency, with immutable, deploy-time fee splits. `collectFees(uint256[] tokenIds)` permissionlessly collects each position and pushes independent basis-point shares of the quote side and the token side, whichever pool position each occupies; native shares are force-sent. Positions sent to it are irrecoverable by design.
- **BeneficiaryVault** — a claim recipient whose transferable ERC721 represents a position's fee beneficiary. `registerBeneficiary(tokenId, beneficiary)` is authorized by position custody, so a depositor registers BEFORE transferring the position to a terminal custodian like the FeeSplitter. Unregistered positions pay out to immutable per-side fallbacks, routed by the vault's configured quote currency.
...
Track FeeSplitter deploy addresses and read `getSplits()` for recipients. FeeSplitter serves pools that pair its immutable `quoteCurrency` on either side (native ETH on current deployments). BuybackAndBurn burns the position's `currency0` or `currency1`, selected per deployment, and the burn currency must be an ERC20. See the [periphery README](../src/periphery/README.md) for contract details.
