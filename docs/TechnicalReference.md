...
#### FeeSplitter
The `FeeSplitter` is a singleton custodian of native-ETH v4 LP positions with immutable, deploy-time fee splits. `collectFees(uint256[] tokenIds)` permissionlessly collects each position and pushes independent basis-point shares of native ETH (`currency0`) and token (`currency1`) in a single pass; native shares are force-sent. Position deposits may carry `abi.encode(PositionCallbackData[])`: each entry targets one `positionCallback` recipient by its immutable `getSplits()` index, with no broadcast and empty data meaning no registration. A `feesCallback` recipient receives one two-amount `onFeesReceived` call after both transfers; failures are swallowed. `BeneficiaryVault` owns beneficiary registration: it inherits BaseLPFeesPositionRecipient's accounting, mints a transferable beneficiary NFT after a custody proof, gates collection to its NFT owner, and flushes unregistered shares to per-side fallbacks. Base recipients apply their `_beforeTransfer` policy per currency; EOAs skip the optional executor callback, so security lives in before/after outcome checks. `DirectLaunchStrategy` requires an abi-encoded `DirectLaunchConfig` naming the fee beneficiary in `configData`; its vault rejects itself at registration. Positions held by the splitter are irrecoverable by design — there is no owner, operator, or exit path.
