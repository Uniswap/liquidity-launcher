...
### LBPStrategy `recoveryDelay`

`LBPStrategy`'s constructor takes an immutable `recoveryDelay` (in blocks) — the wait past `migrationBlock` before `recoverFunds` can be called. Pick a per-chain value that corresponds to roughly the wall-time you want (e.g. `7_200` for ~1 day on a 12s-block chain).
