...
| [`FeeSplitter`](./FeeSplitter.sol) | Singleton, zero-admin custodian of v4 native-ETH LP positions. Permissionless `collectFees(tokenIds[])` pushes immutable independent bps splits in one pass; native shares are force-sent. Deposits may carry `abi.encode(PositionCallbackData[])`, which targets opt-in recipients by immutable split index (no broadcast); fee callbacks receive both amounts once and failures are swallowed. Positions are irrecoverable by design. |
| [`BeneficiaryVault`](./BeneficiaryVault.sol) | A `BaseLPFeesPositionRecipient` with transferable beneficiary NFTs. Its custody-proven deposit callback registers the beneficiary; only that NFT owner may collect attributed fees, while unregistered shares are flushed to immutable per-side fallbacks. |
