...
| [`FeeSplitter`](./FeeSplitter.sol) | Singleton, zero-admin custodian of v4 native-ETH LP positions. Permissionless `collectFees(tokenIds[])` pushes immutable independent bps splits in one pass; native shares are force-sent. Deposits carry no data — the splitter learns nothing about its recipients; fee callbacks receive both amounts once and a failing notification reverts that position's collect, leaving its fees unrealized in the pool (failures are position-scoped, so the other token IDs in a batch can still be collected on their own). `currency1` must be a standard token: a fee-on-transfer or rebasing `currency1` makes that position's collects revert. Positions are irrecoverable by design. |
