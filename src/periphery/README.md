...
| [`BuybackAndBurnPositionRecipient`](./BuybackAndBurnPositionRecipient.sol) | Singleton recipient for native-ETH-paired positions. A permissionless `claim(tokenId, minCurrency0Amount, minCurrency1Amount)` entrypoint pays the caller both attributed sides, then pulls a minimum amount of the position's `currency1` back from the caller and burns it. Designed so MEV searchers can profitably trigger buyback-and-burns. |
| [`FeeSplitter`](./FeeSplitter.sol) | Singleton, zero-admin custodian of v4 native-ETH LP positions. Permissionless `collectFees(tokenIds[])` collects each position and pushes independent, immutable bps splits of the ETH and token sides to fixed recipients. A fee-beneficiary sentinel recipient resolves per position to the beneficiary registered when the position was deposited (via the PositionManager `safeTransferFrom` data); unregistered shares go to a per-side fallback and native shares are force-sent. Positions are irrecoverable by design (no operator, no exit path). |
