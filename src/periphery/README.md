...
| [`BeneficiaryVault`](./BeneficiaryVault.sol) | A `BasePositionRecipient` with transferable beneficiary NFTs. `registerBeneficiary(tokenId, beneficiary)` is authorized by position custody, so depositors register directly with the vault BEFORE transferring the position away; only the NFT owner may claim attributed amounts, while unregistered shares are flushed to immutable per-side fallbacks. |
