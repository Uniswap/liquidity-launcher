...
| [`FeeSplitter`](./FeeSplitter.sol) | Singleton, zero-admin custodian of v4 LP positions pairing its immutable quote currency (native ETH or an ERC20) on either side. Permissionless `collectFees(tokenIds[])` pushes immutable, independent bps splits of quote-side and token-side fees; native shares are force-sent. A recipient that reverts its callback reverts the whole call, so callers should exclude such token IDs — the skipped fees stay collectable in the pool. The token side must be a standard, unrestricted token. Positions are irrecoverable by design. |
| [`BeneficiaryVault`](./BeneficiaryVault.sol) | A `BaseClaimRecipient` with transferable beneficiary NFTs. `registerBeneficiary(tokenId, beneficiary)` is authorized by position custody, so depositors register BEFORE transferring the position away. Only the NFT owner may claim attributed amounts; unregistered shares pay out to immutable per-side fallbacks, routed by whether each side is the vault's configured quote currency. |
...
| [`BuybackAndBurnClaimRecipient`](./BuybackAndBurnClaimRecipient.sol) | Callback recipient: `claim` pays the attributed amounts to the caller, invokes `onClaimed` (during which the caller may perform a buyback), then pulls `minBurnAmount` of the position's burn currency from the caller and sends it to the burn address. The immutable `burnCurrency0` selects the burn currency (`currency0` when true, `currency1` when false), which must be an ERC20, not native ETH. Designed so MEV searchers can profitably trigger buyback-and-burns. |
