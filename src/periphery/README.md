...
| [`FeeSplitter`](./FeeSplitter.sol) | Singleton, zero-admin custodian of v4 native-ETH LP positions. Permissionless `collectFees(tokenIds[])` pushes immutable independent bps splits in one pass; native shares are force-sent. Deposits carry no data — the splitter learns nothing about its recipients; fee callbacks receive both amounts once and failures are swallowed. Positions are irrecoverable by design. |
