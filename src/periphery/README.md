...
| [`AutoCompoundPositionRecipient`](./AutoCompoundPositionRecipient.sol) | Singleton recipient that permissionlessly compounds fees for arbitrary held v4 positions. Pool currencies are derived per token ID, while caller reward, growth cap, operator, and timelock configuration are shared across every position. |
