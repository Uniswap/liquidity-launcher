...
| [`VestingClaimRecipient`](./VestingClaimRecipient.sol) | Rate-limiting middle hop. Custodies beneficiary NFTs so it becomes the only party a vault will pay, drains them through permissionless `claimFor(source, tokenId, …)`, then releases at most `maxCurrencyPerBlock` per block per position to a recipient fixed at deploy. The first `claim` for a tokenId only starts its clock and releases nothing. The cap bounds one position's rate, not the contract's aggregate rate across positions. |
