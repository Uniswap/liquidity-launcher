...
| [`FeeSplitter`](./FeeSplitter.sol) | Singleton, zero-admin custodian of v4 native-ETH LP positions. Permissionless `collectFees(tokenIds[])` collects each position and pushes independent, immutable bps splits of the ETH and token sides to fixed recipients. The splitter is itself an ERC721: depositing a position with a beneficiary in the `safeTransferFrom` data mints a beneficiary NFT (same tokenId) to them, and a fee-beneficiary sentinel recipient resolves per position to that NFT's current holder — the fee stream transfers with the NFT. Unregistered or undeliverable shares go to a per-side fallback. Positions are irrecoverable by design (no operator, no exit path). |
