...
| Contract | Purpose |
| --- | --- |
| [`TimelockedPositionRecipient`](./TimelockedPositionRecipient.sol) | Holds one or more v4 LP positions until a timelock block is reached, then approves a configured operator to transfer them. Base contract for the recipient below. |
| [`BuybackAndBurnClaimRecipient`](./BuybackAndBurnClaimRecipient.sol) | Adds a permissionless `claim(tokenId, minCurrency0Amount, minCurrency1Amount)` entrypoint that (a) pulls a minimum amount of `token` from the caller and burns it, (b) pays out the attributed amounts, (c) forwards the `token` side to the burn address, (d) forwards the `currency` side to the caller. Designed so MEV searchers can profitably trigger buyback-and-burns. |
| [`FeeSplitter`](./FeeSplitter.sol) | Singleton, zero-admin custodian of v4 native-ETH LP positions. Permissionless `collectFees(tokenIds[])` pushes immutable independent bps splits in one pass; native shares are force-sent. Deposits carry no data — the splitter learns nothing about its recipients; fee callbacks receive both amounts once, and a failing notification reverts that position's collect, leaving its fees in the pool while other token IDs stay collectable. `currency1` must be a standard, unrestricted token. Positions are irrecoverable by design. |
| [`BeneficiaryVault`](./BeneficiaryVault.sol) | A `BaseClaimRecipient` with transferable beneficiary NFTs. `registerBeneficiary(tokenId, beneficiary)` is authorized by position custody, so depositors register directly with the vault BEFORE transferring the position away; only the NFT owner may claim attributed amounts, while unregistered shares are flushed to immutable per-side fallbacks. |
| [`UERC20BeneficiaryVault`](./UERC20BeneficiaryVault.sol) | A `BeneficiaryVault` for positions whose custodian never registered a beneficiary, e.g. those held by strategies that predate the vault. The creator of the launcher-created UERC20 in the pair proves themselves through the token's `graffiti()`, and that first proven claim mints them the beneficiary NFT, so every later claim runs the base's plain owner check. An unregistered position pairing such a token reverts for anyone else rather than flushing the creator's share to the fallbacks. |
