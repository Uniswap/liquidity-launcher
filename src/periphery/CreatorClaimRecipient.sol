...
///         creator hashed into the paired launcher-created token's graffiti. The first proven claim
///         mints the creator a transferable ERC721 representing the claim right.
/// @dev Native-only: the token side is never paid out, so only route native fee shares here.
contract CreatorClaimRecipient is ICreatorClaimRecipient, BaseClaimRecipient, ERC721 {
    ...
    /// @dev Requires a native currency0. An unminted position requires a currency1 graffiti matching the
    ///      caller's hash — the derivation of LiquidityLauncher.getGraffiti — and mints them the claim
    ///      NFT; ownership can never return to zero, so every later claim runs the owner check alone.
    ///      The graffiti is read with a raw staticcall so a currency1 that answers with anything but a
    ///      single word reports "not a creator" instead of reverting the claim with an uncatchable
    ///      decode error.
    function _beforeClaimTransfer(
        ...
        if (owner == address(0)) {
            (bool success, bytes memory data) =
                Currency.unwrap(_currency1).staticcall(abi.encodeCall(IUERC20.graffiti, ()));
            ...
            revert NotClaimOwner(_tokenId, msg.sender);
        }
