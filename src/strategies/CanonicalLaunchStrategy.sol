...
contract CanonicalLaunchStrategy is DirectLaunchStrategy, BlockNumberish {
    ...
    constructor(
        ...
        if (
            _launcher == address(0) || address(_positionManager) == address(0) || address(_poolManager) == address(0)
                || _launchHook == address(0) || _dynamicFeeModule == address(0)
        ) revert ZeroAddress();
        ...
        nonReentrant
    {
        ...
        (PoolId poolId,,) = _launch(token, TOTAL_SUPPLY, params, balanceBefore);

        emit CanonicalTokenLaunched(poolId, token, address(positionRecipient));
    }
    ...
    function _launchParameters(address positionRecipient)
        internal
        view
        virtual
